...
public aspect Instrumentator {
    private final ArrayList<String> umlsrc = new ArrayList<String>();
    private int callDepth;
    private final String PATH = "C:\\Users\\Sraddha Bhattacharje\\IdeaProjects\\APP_Assignment_AspectJ\\sequence.txt";
    ...
    pointcut Detect(): !within(Instrumentator) && (execution(public * *.*(..)));
    ...
    after()returning(Object r): Detect(){
        String className;
        try {
            System.out.println(thisJoinPoint.getThis().getClass().getName());
            className = thisJoinPoint.getThis().getClass().getName();
